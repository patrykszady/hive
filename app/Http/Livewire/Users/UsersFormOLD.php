...
class UsersForm extends Component
{
        ...
        if($user_exists){
            ...
            if($this->user_add_type->getTable() == 'vendors'){
                if($this->user_vendor_id == "NEW"){
                    ...
                    $this->user->type = 'Sub';
                    $this->user->role = 1;
                    $this->via_vendor->business_name = $this->model['business_name'];
                }else{
    ...
    public function newMember($model, $id = NULL)
    {
        $type = $model['business_type'] ? 'vendor' : $model;
        $this->model = $model;
        //if numeric, adding to existing, if NEW, creating new Vendor or Client
        if($type == 'client'){
            $user_add_type = Client::find($id);

            //when creating new Client
            // if(is_null($user_add_type)){
            //     $user_add_type = Client::make();
            // }
        }elseif($type == 'vendor'){
            $user_add_type = Vendor::find($id);

            //when creating new Vendor
            if(is_null($user_add_type)){
                $user_add_type = Vendor::make();
                $this->user_vendor_id = "NEW";
                // $this->user_add_type->type = $model['business_type'];
            }
        }
